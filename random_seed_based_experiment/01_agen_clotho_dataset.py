...
def process_audio_data(
    dataset_dirs, audio_splits, replication_factor, text_files, seed=None
):
    """
    Processes audio data to extract file IDs (fids) and durations, with random selection of synthetic copies
    for the development split. Only processes audios listed in the text files.
    ...
    for split in audio_splits:
        ...
        # Read the corresponding text file for this split
        text_fpath = text_files.get(split)
        text_data = pd.read_csv(text_fpath) if text_fpath else None

        # Extract the filenames listed in the text file
        valid_filenames = (
            set(text_data["fname"].str.strip())
            ...
            for fpath in tqdm(
                ...
                try:
                    ...
                    if fname not in valid_filenames:
                        continue  # Skip files not listed in the text file
...
if __name__ == "__main__":
    for split in range(1, 11):
        ...
        global_params = {
            ...
            "text_files": {
                "development": f"data/random_seed_experiment/agen_captions_chunk_{split}.csv",
                "validation": "data/Clotho/validation_captions.csv",
                "evaluation": "data/Clotho/evaluation_captions.csv",
            },
        }

        replication_factor = 3  # User-defined value for how many times to pick from synthetic copies (1 to 5)

        output_dir = f"./data/random_seed_experiment/seed_split_{split}_replication_{replication_factor}_88"
        ...
        # 24 for each and every benchmark
        # 42, 88 for random_seed split experiment
