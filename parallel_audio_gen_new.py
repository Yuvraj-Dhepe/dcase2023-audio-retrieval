...
def process_chunk(df_chunk: pd.DataFrame, output_folder: Path):
    ...
    for _, row in tqdm(df_chunk.iterrows(), total=df_chunk.shape[0], desc="Processing Captions"):
...
def process_csv(csv_file_path: str, output_folder: Path):
    ...
    cluster = LocalCluster(n_workers=3, threads_per_worker=1, memory_limit='16GB',  dashboard_address=':8797',resources={'GPU': 1})
    client = Client(cluster)

    futures = []
    for chunk, folder in zip(chunks, output_folders):
        future = client.submit(process_chunk, chunk, folder, resources = {'GPU':1})
...
if __name__ == "__main__":
    process_csv('./data/Clotho/evaluation_gen/clotho_captions_evaluation.csv',
