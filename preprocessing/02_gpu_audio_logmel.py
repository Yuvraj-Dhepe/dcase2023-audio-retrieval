...
class LogMelSpectrogram(nn.Module):
    def __init__(self, sample_rate=44100, window_length_secs=0.025, hop_length_secs=0.010, num_mels=128, log_offset=0.0):
        ...
        self.mel_filter_bank = nn.Parameter(torch.tensor(librosa.filters.mel(sr=sample_rate, n_fft=self.fft_length, n_mels=num_mels), dtype=torch.float32), requires_grad=False)

    def forward(self, y):
        y = y.unsqueeze(0)  # Add batch dimension
        spectrogram = torch.stft(y, n_fft=self.fft_length, hop_length=self.hop_length, win_length=self.window_length, window=torch.hann_window(self.window_length).to(y.device), return_complex=True)
...
def process_audio(fpath, fname2fid, device):
    ...
    log_mel_model = LogMelSpectrogram(sample_rate=sr, window_length_secs=0.040, hop_length_secs=0.020, num_mels=64, log_offset=np.spacing(1))
