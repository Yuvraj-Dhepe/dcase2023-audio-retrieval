...
def process_db(db_path, dataset_name, way):
    ...
    with shelve.open(db_path) as stream:
        for qid, items in tqdm(stream.items(), desc=f"Processing {way} for {dataset_name}"):
            R1, R5, R10, mAP = measure_single_item(qid, items, dataset_name, way)
            total_R1 += R1
            total_R5 += R5
            total_R10 += R10
            total_mAP += mAP
    ...
    print(f"{dataset_name} {way} mAP: {avg_mAP:.3f}",
          f"R1: {avg_R1:.3f}",
          f"R5: {avg_R5:.3f}",
...
if __name__ == "__main__":
    ...
    for name in data_conf.keys():
        ...
        with shelve.open(tid2items_db_fpath) as tid_stream:
            for tid, items in tqdm(tid_stream.items(), desc=f"Processing Text2Audio results for {name}"):
