...
ckp_fpath = os.path.join("./z_ckpts", "nk56srrr")
...
def measure(qid2items):
    ...
    print(
        "mAP: {:.3f}".format(np.mean(qid_mAP10s)),
...
for name in data_conf:
    ...
    with shelve.open(filename=score_fpath, flag="r", protocol=2) as stream:
        ...
        for fid in stream:
            ...
            for tid in group_scores:
                if tid2items.get(tid, None) is None:
                    tid2items[tid] = [
                        (fid, group_scores[tid], tid2fid[tid] == fid)
                    ]
                else:
        ...
        result_fpath = os.path.join(ckp_fpath, f"{name}_new.t2a_retrieval.csv")
