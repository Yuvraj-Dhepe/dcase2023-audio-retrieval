...
def train_model(conf, run_id=None):
    ...
    # Define maximum number of epochs and saving interval
    max_epoch = param_conf["num_epoch"]
    save_interval = 20  # Save every 15 epochs

    # Perform training
    for epoch in range(start_epoch, max_epoch+1):
        if epoch > 0:
            # Perform model training
            model_utils.tqdm_train(model, train_dl, objective, optimizer, epoch, max_epoch)
...
if __name__ == "__main__":
    ...
    with open("./conf_yamls/exp_5x_conf.yaml", "rb") as stream:
