...
def process_audio_data(dataset_dirs, audio_splits, replication_factor, seed=None):
    """
    ...
    for split in audio_splits:
        ...
        synthetic_files = {}  # Dictionary to store synthetic audio files based on original file names
        ...
        for dataset_dir in used_dataset_dirs:
            ...
            for fpath in tqdm(glob.glob(r"{}/*.wav".format(audio_dir)), desc=f"Processing audio files in {split}"):
                try:
                    ...
                    if clip.info.length > 0.0:  # Ensure the audio file has valid length
                        fid = hashlib.md5(fpath.encode()).hexdigest()  # Generate unique ID for each file
                        fname = os.path.basename(fpath)
                        if "_cap_" not in fname:  # If file is original (not synthetic)
                            original_files.append((fid, fname, clip.info.length))  # Add original file info
                        else:
                            ...
                            synthetic_files[base_name].append((fid, fname, clip.info.length))  # Add synthetic file info
        ...
        if split == "development":
            selected_fid2fname = {}  # Dictionary to store selected file IDs and file names
            selected_durations = []  # List to store selected file durations

            # Iterate over each original file
            for original_fid, original_fname, original_duration in tqdm(original_files, desc="Selecting synthetic copies"):
                ...
                synthetic_candidates = synthetic_files.get(original_fname[:-4], [])
                random.shuffle(synthetic_candidates)  # Shuffle the synthetic candidates randomly
                ...
                for synthetic_fid, synthetic_fname, synthetic_duration in selected_candidates:
...
def process_text_data(text_files, audio_fid2fname, output_dir):
    ...
    for split, text_fpath in text_files.items():
        fid2fname = audio_fid2fname["audio_fid2fname"][split]# Get the file ID to filename mapping for the split
        stripped_fname2fid = {fid2fname[fid].strip(" "): fid for fid in fid2fname} # Map stripped filenames to file IDs
        ...
        for i in tqdm(text_data.index, desc=f"Processing text data in {split}"):
            ...
            fid = stripped_fname2fid.get(fname)  # Get the corresponding file ID
            if fid:
                tid = hashlib.md5((fid + text).encode()).hexdigest()  # Generate a unique text ID (tid)
                tokens = [t for t in re.split(r"\s", text) if len(t) > 0]  # Tokenize the text
                text_rows.append([tid, fid, fid2fname[fid], raw_text, text, tokens])  # Append processed text data

        # Convert the text rows to a DataFrame and save as CSV
        text_rows_df = pd.DataFrame(data=text_rows, columns=["tid", "fid", "fname", "raw_text", "text", "tokens"])
...
def generate_data_statistics(audio_splits, audio_fid2fname, output_dir):
    ...
    for split in audio_splits:
        fid2fname = audio_fid2fname["audio_fid2fname"][split]  # Get file ID to filename mapping for the split
        ...
        for tokens in tqdm(text_data["tokens"], desc=f"Generating data statistics for {split}"):
            bag.extend(tokens)
            vocabulary = vocabulary.union(tokens)  # Update vocabulary with unique tokens
...
if __name__ == "__main__":
    global_params = {
        "dataset_dirs": ["./data/Clotho", "./data/Clotho_caption_1", "./data/Clotho_caption_2", "./data/Clotho_caption_3", "./data/Clotho_caption_4", "./data/Clotho_caption_5"],
        "audio_splits": ["development", "validation", "evaluation"],
        "text_files": {
            "development": "data/exp_2/transformed_development.csv",
    ...
    # Process audio data
    audio_data = process_audio_data(global_params["dataset_dirs"], global_params["audio_splits"], replication_factor, seed=24)
    ...
    data_statistics = generate_data_statistics(global_params["audio_splits"], audio_data, output_dir)
