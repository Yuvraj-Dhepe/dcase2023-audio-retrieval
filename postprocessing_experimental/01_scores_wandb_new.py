...
for name, ds in zip(["val", "eval"], [val_ds, eval_ds]):
    ...
    with shelve.open(
        filename=fid_score_fpath, flag="n", protocol=2
    ) as fid_stream, shelve.open(
        filename=tid_score_fpath, flag="n", protocol=2
    ) as tid_stream:
        ...
        for fid in tqdm(
            ...
            for i in range(0, len(text2vec), batch_size):
                ...
                for j, tid in enumerate(batch_text_ids):
                    ...
                    fid_group_scores[tid] = (
                        xmodal_scores[j].item(),
                        is_relevant,
                    )
                    if tid not in tid_group_scores:
                        tid_group_scores[tid] = []
                    tid_group_scores[tid].append(
                        (fid, xmodal_scores[j].item(), is_relevant)
                    )

            # Save scores for the current audio file (fid)
            fid_stream[fid] = [
                (tid, fid_group_scores[tid][0], fid_group_scores[tid][1])
                for tid in fid_group_scores.keys()
            ]
